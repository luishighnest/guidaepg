...
class SkyScraper {
    ...
    public function getMatchedChannels() {
        ...
        preg_match_all('/href=["\'](\/canali\/[a-zA-Z0-9_\-]+)["\']/i', $html, $matches);
    ...
    private function extractPrograms($html) {
        ...
        if (preg_match('/<script\s+[^>]*id=["\']__NEXT_DATA__["\'][^>]*>(.*?)<\/script>/s', $html, $matches)) {
            try {
                $data = json_decode($matches[1], true);
                $props = $data['props']['pageProps'] ?? [];
                $programs_list = $props['initialData']['channel']['programs'] ?? $props['programs'] ?? [];

                $extracted = [];
                foreach ($programs_list as $p) {
                    $ora_raw = $p['startTime'] ?? $p['ora'] ?? "";
                    $ora = "";
                    ...
                    $titolo_raw = $p['title'] ?? $p['titolo'] ?? "N/A";
                    $desc_raw = trim($p['description'] ?? $p['descrizione'] ?? $p['desc'] ?? "");

                    $detail_url = $p['link'] ?? $p['url'] ?? $p['href'] ?? null;
                    if (!$detail_url && isset($p['slug'])) {
