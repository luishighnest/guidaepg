...
<body class="dashboard-body">
  ...
  <!-- EPG iniettato lato server da PHP: dati disponibili immediatamente, senza fetch() iniziale -->
  <script>
  ...
  <div class="dashboard-layout">
    ...
    <aside class="dash-sidebar">
      ...
      <div class="dash-cat-title">Categorie</div>
    ...
    <button id="sidebar-toggle" class="sidebar-toggle" title="Nascondi/Mostra Sidebar">
      ...
      <div class="dash-info-card">
        ...
        <div id="player-ch-now-container">
          <div class="dash-info-now-box">
            <div style="display: flex; align-items: center; justify-content: space-between;">
              ...
              <span style="font-family: var(--font-alt); font-size: 0.8rem; color: #8ca3ba; font-weight: 700;">--:--</span>
            </div>
            <div style="font-size: 1.1rem; font-weight: 800; color: #fff; line-height: 1.3;">Programmazione in corso</div>
          </div>
        </div>
        ...
        <div id="player-ch-next-container">
          <div class="dash-info-next-box">
            ...
            <div style="font-size: 0.95rem; font-weight: 700; color: #e2e8f0; line-height: 1.3;">Programma successivo in arrivo</div>
          </div>
        </div>
      </div>
    </div>
    ...
    <!-- Panels -->
    <div class="dash-panel dash-guide-panel" style="grid-column: 2 / -1;">
        ...
        <div style="display: flex; align-items: center; gap: 1rem;">
          <div class="dash-search-container" style="position: relative; width: 300px;">
            ...
            <input type="text" id="dash-search" placeholder="Cerca canale, categoria o evento..."
              style="width: 100%; background: #1a1a24; border: 1px solid var(--border-subtle); border-radius: 8px; padding: 6px 12px 6px 36px; color: #fff; font-size: 0.85rem;" />
          </div>

          <button id="guide-expand-toggle" class="dash-btn-toggle" title="Espandi/Riduci Guida TV"
            style="background: rgba(255, 255, 255, 0.05); border: 1px solid var(--border-subtle); color: var(--text-secondary); padding: 6px 14px; border-radius: 8px; cursor: pointer; font-size: 0.8rem; font-weight: 700; display: flex; align-items: center; gap: 6px; transition: all 0.2s;">
            <i class="ph ph-arrows-out-simple"></i> <span>Espandi</span>
          </button>
        </div>
      </div>
  ...
  <script>
    ...
    let currentCat = currentChannel ? currentChannel.cat : 'sport';

    if (!currentChannel) {
      const sportChannels = getChannelsByCategory('sport');
      currentChannel = sportChannels.length > 0 ? sportChannels[0] : CHANNELS[0];
      currentCat = currentChannel.cat;
    }
    ...
    Object.keys(CATEGORIES).forEach(key => {
      ...
      a.innerHTML = `<i class="ph ${c.icon}" style="color: ${key === currentCat ? '#fff' : c.color}"></i> ${c.label} <span class="dash-cat-count">${count}</span>`;
      a.onclick = (e) => {
        e.preventDefault();
        renderChannels(key);
        restoreIconColors();
    ...
    aAll.innerHTML = `<i class="ph ph-squares-four" style="color: ${'all' === currentCat ? '#fff' : '#FFFF00'}"></i> Tutte <span class="dash-cat-count">${allCount}</span>`;
    aAll.onclick = (e) => {
      e.preventDefault();
      renderChannels('all');
      restoreIconColors();
    ...
    function renderChannels(catKey, searchQuery = '') {
      ...
      for (let i = 0; i < filtered.length; i++) {
        ...
        if (isPlaying) html += `<span class="dash-ch-live-badge">▶ IN ONDA</span>`;
        ...
        if (currentProgram) {
          ...
          html += `<span style="font-weight:700;font-size:0.95rem;color:${isPlaying ? '#ef4444' : '#fff'};white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${currentProgram.titolo}</span>`;
          html += `<span style="font-family:var(--font-alt);font-size:0.8rem;color:#60a5fa;font-weight:700;flex-shrink:0">${currentProgram.ora}</span></div>`;
          ...
          if (channelEpg.next) {
            html += `<div class="dash-ch-col-next"><div style="display:flex;align-items:center;gap:6px">`;
            html += `<span style="font-size:0.65rem;font-weight:800;padding:1px 5px;border-radius:3px;background:rgba(96,165,250,0.1);color:#60a5fa;border:1px solid rgba(96,165,250,0.2);letter-spacing:0.5px">DOPO</span>`;
            html += `<span style="font-family:var(--font-alt);font-size:0.72rem;color:var(--text-muted);font-weight:700">${channelEpg.next.ora}</span></div>`;
            html += `<div style="font-size:0.8rem;font-weight:600;color:var(--text-secondary);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;margin-top:1px">${channelEpg.next.titolo}</div></div>`;
          } else {
            html += `<div class="dash-ch-col-next" style="display:flex;align-items:center;justify-content:flex-start"><span style="font-size:0.75rem;color:var(--text-muted);font-style:italic"><i class="ph ph-info" style="margin-right:4px;vertical-align:middle"></i>Fine trasmissioni</span></div>`;
          }
        } else {
          ...
          html += `<div class="dash-ch-col-next" style="display:flex;align-items:center;justify-content:flex-start"><span style="font-size:0.75rem;color:var(--text-muted);font-style:italic"><i class="ph ph-calendar" style="margin-right:4px;vertical-align:middle"></i>Canale Live Continuo</span></div>`;
        }
      ...
      if (!epg.now) {
        ...
        nowContainer.innerHTML = `
          <div class="dash-info-now-box">
            <div style="display:flex;align-items:center;justify-content:space-between;">
              ...
              <span style="font-family:var(--font-alt);font-size:0.8rem;color:#8ca3ba;font-weight:700;">Live</span>
            </div>
            <div style="font-size:1.1rem;font-weight:800;color:#fff;line-height:1.3;margin-top:2px;">Eventi ${catMeta ? catMeta.label : currentChannel.cat}</div>
          </div>`;
        nextContainer.innerHTML = `
          <div class="dash-info-next-box">
            ...
            <div style="font-size:0.95rem;font-weight:700;color:#e2e8f0;line-height:1.3;margin-top:2px;">Programmazione live continua</div>
          </div>`;
        return;
      }
      ...
      nowContainer.innerHTML = `
        <div class="dash-info-now-box">
          <div style="display:flex;align-items:center;justify-content:space-between;">
            ...
            <span style="font-family:var(--font-alt);font-size:0.8rem;color:#60a5fa;font-weight:700;">${epg.now.ora}</span>
          </div>
          <div style="font-size:1.1rem;font-weight:800;color:#fff;line-height:1.3;margin-top:4px;">${epg.now.titolo}</div>
      ...
      if (epg.next) {
        nextContainer.innerHTML = `
          <div class="dash-info-next-box">
            <div style="display:flex;align-items:center;justify-content:space-between;">
              <span style="color:#60a5fa;font-weight:800;text-transform:uppercase;letter-spacing:1px;font-size:0.7rem;background:rgba(96,165,250,0.1);padding:2px 8px;border-radius:4px;">A SEGUIRE</span>
              <span style="font-family:var(--font-alt);font-size:0.8rem;color:#8ca3ba;font-weight:700;">${epg.next.ora}</span>
            </div>
            <div style="font-size:0.95rem;font-weight:700;color:#e2e8f0;line-height:1.3;margin-top:4px;">${epg.next.titolo}</div>
